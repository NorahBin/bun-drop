...
function SignInComponent({ setUser }) {
  //Statevariabler för username, password
  const [username, setUsername] = useState("");
  const [password, setPassword] = useState("");
  const navigate = useNavigate();

  //Funktion för att hantera username input change
  ...
  async function handleSubmitForm(e) {
    ...
    try {
      ...
      const users = await response.json(); //Parse:ar response till json
      const user = users.find((u) => u.name === username); //Kollar om username och password matchar någon user från fetchad data

      if (user && user.password === password) {
        console.log("User signed in successfully:", user);
        setUser(user); //Sätter userstate
        localStorage.setItem("user", JSON.stringify(user)); //Sparar user i local storage så att den hålls inloggad
